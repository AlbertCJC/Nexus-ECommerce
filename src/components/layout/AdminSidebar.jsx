...
const navItems = [
  ...
  { path: '/admin/brands', label: 'Brands', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg> },
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-[rgb(var(--bg-card))] border-r border-[rgb(var(--border-subtle))] transform transition-transform duration-200 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`} aria-label="Admin navigation">
      ...
      <nav className="p-4 space-y-1" aria-label="Admin menu">
        {navItems.map(item => (
          <Link key={item.path} to={item.path} onClick={onClose} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${location.pathname === item.path ? 'bg-[rgb(var(--accent-primary))/0.1] text-[rgb(var(--accent-primary))]' : 'text-[rgb(var(--text-secondary))] hover:bg-[rgb(var(--bg-hover))] hover:text-[rgb(var(--text-primary))]'}`}>
            {item.icon}
            {item.label}
          </Link>
        ))}
