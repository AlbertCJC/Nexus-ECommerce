...
export function CartItem({ item, product, onUpdateQuantity, onRemove }) {
  ...
  return (
    <div className="flex gap-4 p-4 card">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 mt-2">
          <div className="flex items-center border border-[rgb(var(--border-subtle))] rounded-lg bg-[rgb(var(--bg-elevated))]">
            <button onClick={() => handleQuantityChange(-1)} disabled={item.quantity <= 1} className="px-3 py-1 text-[rgb(var(--text-secondary))] hover:bg-[rgb(var(--bg-hover))] disabled:opacity-50 transition-colors" aria-label="Decrease quantity">−</button>
            <span className="px-3 py-1 text-sm font-medium w-10 text-center border-x border-[rgb(var(--border-subtle))]">{item.quantity}</span>
            <button onClick={() => handleQuantityChange(1)} disabled={item.quantity >= maxQuantity} className="px-3 py-1 text-[rgb(var(--text-secondary))] hover:bg-[rgb(var(--bg-hover))] disabled:opacity-50 transition-colors" aria-label="Increase quantity">+</button>
          </div>
      ...
      <button onClick={handleRemove} className="p-2 text-[rgb(var(--text-muted))] hover:text-[rgb(var(--accent-danger))] transition-colors" aria-label={`Remove ${product.name} from cart`}>
